...
class TodoRepositoryTest extends BaseTest {
	...
	public void When_FindTodoByDate_Expect_DoneTodoList() {
		// given
		Todo todo1 = this.createTodo("todo1", StatType.생존력, LocalDateTime.now(), false, true, false); // 완료하지 않은 투두
	...
	public void When_FindTodoByDateAndAlarmIsTrue_Expect_AlarmTodoList() {
		...
		Todo todo1 = this.createTodo("todo1", StatType.생존력, LocalDateTime.of(now, time.withSecond(0)), false, true,
			false); // 완료하지 않은 투두
		Todo todo2 = this.createTodo("todo2", StatType.인싸력, LocalDateTime.of(now, time.withSecond(59)), true, true,
			false); // 완료한 투두
		Todo todo3 = this.createTodo("todo3", StatType.갓생력, LocalDateTime.of(now, time), true, false, false); // 완료한 투두
		Todo todo4 = this.createTodo("todo4", StatType.창의력, LocalDateTime.of(2023, 10, 30, 13, 30), true, true, false);
		// 삭제한 투두
		Todo todo5 = this.createTodo("todo5", StatType.최애력, LocalDateTime.of(now, time), false, true, true);
		todoRepository.saveAll(List.of(todo1, todo2, todo3, todo4, todo5));

		// when
		List<Todo> todoList = todoRepository.findTodosByStatusIsFalseAndTodoDateIsBetweenAndAlarmIsTrue(
			LocalDateTime.of(now, time.withSecond(0)), LocalDateTime.of(now, time.plusMinutes(1).withSecond(0)));
